...
pygame.display.set_caption("PONG") #başlık ekle
#define font
...
class paddle():
    ...
    def ai(self):
        # ai to move the paddle automaticly
        # move down
...
class ball():
    ...
    def move(self):
        # add collison detextion
        #check collision with top margin
        if self.rect.top < margin:
            self.speed_Y*=-1
        if self.rect.bottom >screen_height:
            self.speed_Y*=-1
        #check collison with paddles
    ...
    if live_ball==True:
        ...
        if winner==0:
            ...
            if winner==1:
                ...
                cpu_score+=1
    #print player insctructions
    if live_ball==False:
        if winner==0:
            draw_text('CLICK ANYWHERE TO START',font,white,100,screen_height//2-100)

        if winner==1:
            draw_text('YOUR SCORED!', font, white, 220, screen_height // 2 - 100)
